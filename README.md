...
The UI listens on `127.0.0.1:23560` by default. Saved worker configs are stored in a SQLite database under `./baseload-config`.
...
- `ATLAS_BASELOAD_DB_PATH`: SQLite database path for saved worker configs, defaults to `baseload-config/baseload.sqlite` locally and `/app/baseload-config/baseload.sqlite` in Docker.
- `ATLAS_BASELOAD_DATA_DIR`: Docker Compose host directory mounted at `/app/baseload-config`, defaults to `./baseload-config`.
